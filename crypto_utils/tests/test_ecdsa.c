...
void test_file_signature() {
  ...
  assert(ecdsa_sign(&curve, sign_private, sizeof(sign_private), file_hash,
                    sizeof(file_hash), &signature) == 0);
  ...
  // 验证签名
  assert(ecdsa_verify(&curve, sign_public, sizeof(sign_public), file_hash,
                      sizeof(file_hash), &signature) == 0);
...
void test_ecdsa_with_ecdh() {
  ...
  assert(ecdh_generate_keypair(&curve, alice_ecdh_public,
                               sizeof(alice_ecdh_public), alice_ecdh_private,
                               sizeof(alice_ecdh_private)) == 0);

  // Alice生成用于签名的ECDSA密钥对
  uint8_t alice_sign_public[PUBKEY_SERIALIZED_LEN];
  uint8_t alice_sign_private[32];
  assert(ecdsa_generate_keypair(&curve, alice_sign_public,
                                sizeof(alice_sign_public), alice_sign_private,
                                sizeof(alice_sign_private)) == 0);
  ...
  assert(ecdh_compute_secret(&curve, alice_ecdh_public,
                             sizeof(alice_ecdh_public), bob_ecdh_private,
                             sizeof(bob_ecdh_private), bob_secret,
                             sizeof(bob_secret)) == 0);

  // 对共享密钥进行签名
  ECDSASignature signature;
  assert(ecdsa_sign(&curve, alice_sign_private, sizeof(alice_sign_private),
                    alice_secret, sizeof(alice_secret), &signature) == 0);

  // Bob验证签名和共享密钥
  assert(ecdsa_verify(&curve, alice_sign_public, sizeof(alice_sign_public),
                      bob_secret, sizeof(bob_secret), &signature) == 0);
