...
void test_keypair_generation_and_formats() {
  ...
  assert(ecdh_generate_keypair(&curve, public_key, sizeof(public_key),
                               private_key,
                               sizeof(private_key)) == ECDH_SUCCESS);
  ...
  assert(ecdh_deserialize_pubkey(&curve, &pubkey, public_key,
                                 sizeof(public_key)) == ECDH_SUCCESS);
  assert(ecdh_serialize_pubkey_compressed(
             &pubkey, compressed_key, sizeof(compressed_key)) == ECDH_SUCCESS);
...
void test_key_exchange_with_hashing() {
  ...
  assert(ecdh_generate_keypair(&curve, alice_public, sizeof(alice_public),
                               alice_private,
                               sizeof(alice_private)) == ECDH_SUCCESS);
  ...
  assert(ecdh_generate_keypair(&curve, bob_public, sizeof(bob_public),
                               bob_private,
                               sizeof(bob_private)) == ECDH_SUCCESS);

  // 转换为压缩格式
  ECPoint pubkey;
  ec_init_point(&pubkey);
  assert(ecdh_deserialize_pubkey(&curve, &pubkey, alice_public,
                                 sizeof(alice_public)) == ECDH_SUCCESS);
  assert(ecdh_serialize_pubkey_compressed(&pubkey, alice_public_compressed,
                                          sizeof(alice_public_compressed)) ==
         ECDH_SUCCESS);

  assert(ecdh_deserialize_pubkey(&curve, &pubkey, bob_public,
                                 sizeof(bob_public)) == ECDH_SUCCESS);
  assert(ecdh_serialize_pubkey_compressed(&pubkey, bob_public_compressed,
                                          sizeof(bob_public_compressed)) ==
         ECDH_SUCCESS);
  ...
  assert(ecdh_compute_secret(&curve, bob_public, sizeof(bob_public),
                             alice_private, sizeof(alice_private), alice_secret,
                             sizeof(alice_secret)) == ECDH_SUCCESS);

  assert(ecdh_compute_secret(&curve, alice_public, sizeof(alice_public),
                             bob_private, sizeof(bob_private), bob_secret,
                             sizeof(bob_secret)) == ECDH_SUCCESS);
  ...
  assert(memcmp(alice_hashed_secret, bob_hashed_secret, SHA256_DIGEST_SIZE) ==
         0);

  dump_hex("Alice的原始共享密钥", alice_secret, sizeof(alice_secret));
  dump_hex("Alice的哈希共享密钥", alice_hashed_secret,
           sizeof(alice_hashed_secret));
...
void test_error_handling() {
  ...
  assert(ecdh_generate_keypair(&curve, public_key, 10, // 公钥缓冲区太小
         ...
         ECDH_ERROR_INVALID_PARAM);

  // 测试无效的公钥格式
  public_key[0] = 0x00; // 无效的格式标识符
  assert(ecdh_compute_secret(&curve, public_key, sizeof(public_key),
                             private_key, sizeof(private_key), shared_secret,
                             sizeof(shared_secret)) ==
         ECDH_ERROR_SERIALIZE_FAILED);
