...
int ecdh_generate_keypair(ECurve *curve, uint8_t *public_key_out,
                          size_t public_key_len, uint8_t *private_key_out,
                          size_t private_key_len) {
  if (!curve || !public_key_out || !private_key_out ||
      public_key_len < PUBKEY_SERIALIZED_LEN || private_key_len < 32) {
    return ECDH_ERROR_INVALID_PARAM;
  }
...
int ecdh_deserialize_pubkey(ECurve *curve, ECPoint *pubkey, const uint8_t *data,
                            size_t len) {
...
int ecdh_serialize_pubkey(const ECPoint *pubkey, uint8_t *out, size_t out_len) {
...
int ecdh_serialize_pubkey_compressed(const ECPoint *pubkey, uint8_t *out,
                                     size_t out_len) {
  ...
  // 设置压缩格式标记
  out[0] = mpz_cmp_ui(temp, 0) == 0 ? PUBKEY_FORMAT_COMPRESSED_EVEN
...
int ecdh_compute_secret(ECurve *curve, const uint8_t *their_public,
                        size_t their_public_len, const uint8_t *my_private,
                        size_t my_private_len, uint8_t *secret_out,
                        size_t secret_len) {
  if (!curve || !their_public || !my_private || !secret_out ||
      their_public_len < PUBKEY_SERIALIZED_LEN || my_private_len < 32 ||
  ...
  // 反序列化对方的公钥
  if (ecdh_deserialize_pubkey(curve, &their_pubkey, their_public,
                              their_public_len) != ECDH_SUCCESS) {
