...
int ecdsa_init_context(ECurve *curve) { return ecdh_init_context(curve); }

int ecdsa_generate_keypair(ECurve *curve, uint8_t *public_key,
                           size_t public_key_len, uint8_t *private_key,
                           size_t private_key_len) {
  return ecdh_generate_keypair(curve, public_key, public_key_len, private_key,
                               private_key_len);
}

int ecdsa_sign(ECurve *curve, const uint8_t *private_key,
               size_t private_key_len, const uint8_t *hash, size_t hash_len,
               ECDSASignature *signature) {
  if (!curve || !private_key || !hash || !signature || private_key_len < 32 ||
      hash_len < 32) {
    return -1;
  }
...
static void ec_point_add(ECPoint *result, const ECPoint *p1, const ECPoint *p2,
                         const ECurve *curve) {
  if (p1->infinity) {
...
int ecdsa_verify(ECurve *curve, const uint8_t *public_key,
                 size_t public_key_len, const uint8_t *hash, size_t hash_len,
                 const ECDSASignature *signature) {
  if (!curve || !public_key || !hash || !signature || public_key_len < 65 ||
      hash_len < 32) {
    return -1;
  }
