...
void serialize_ecpoint(const ECPoint *point, unsigned char *buffer,
                       size_t *length) {
  size_t offset = 0;
  size_t x_size = (mpz_sizeinbase(point->x, 2) + 7) / 8;
  size_t y_size = (mpz_sizeinbase(point->y, 2) + 7) / 8;

  // Serialize x
  buffer[offset++] = (unsigned char)x_size;
  mpz_export(buffer + offset, NULL, 1, 1, 0, 0, point->x);
  offset += x_size;

  // Serialize y
  buffer[offset++] = (unsigned char)y_size;
  mpz_export(buffer + offset, NULL, 1, 1, 0, 0, point->y);
  offset += y_size;

  // Serialize infinity
  ((int *)buffer)[offset++] = point->infinity;
...
void deserialize_ecpoint(ECPoint *point, const unsigned char *buffer,
                         size_t length) {
  size_t offset = 0;
  size_t x_size = buffer[offset++];
  size_t y_size = buffer[offset++];

  // Deserialize x
  mpz_import(point->x, x_size, 1, 1, 0, 0, buffer + offset);
  offset += x_size;

  // Deserialize y
  mpz_import(point->y, y_size, 1, 1, 0, 0, buffer + offset);
  offset += y_size;

  // Deserialize infinity
  point->infinity = ((int *)buffer)[offset++];
}
